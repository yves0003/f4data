...
export class Parser {
  ...
  //   const name = this._eat(this._lookahead?.type === "STRING" ? "STRING" : "IDENTIFIER").value;
  ...
  //     const settings = [];
  //     while ((this._lookahead?.type as string) !== "RBRACKET") {
  //       settings.push(this._eat(["STRING", "IDENTIFIER"]).value);
  //       if ((this._lookahead?.type as string) === "COMMA") this._eat("COMMA");
  //     }
  ...
  private _eat(tokenType: (typeof Spec)[0][1] | (typeof Spec)[0][1][]) {
    ...
    //     throw new Error(`Unexpected token: ${tokenType.join(" or ")}, got ${token.type as string}`);
    //   }
    // } else {
...
// export class Parser {
//   private _tokenizer = new Tokenizer();
//   private _lookahead: ReturnType<Tokenizer["getNextToken"]> = null;
...
//     this._eat("COLON");
//     const left = this.parseQualifiedName().join(".");
//     const relationship = this._eat("RELATIONAL_OPERATOR").value;
//     const right = this.parseQualifiedName().join(".");
//     return { type: "Ref", left, relationship, right };
//   }
