...
export class DicTabProvider2 implements TreeDataProvider<Table> {
  ...
  private view?: TreeView<list_tabs>;
  constructor() {}
  setLink(link: string) {
    this.linkDir = link;
  }
  ...
  setView(view: TreeView<list_tabs>) {
    this.view = view;
  }
  revealItem(name: string) {
    const item = this.data.find((i) => i.name === name);
    if (item && this.view) {
      this.view.reveal(item, { select: true, focus: true });
    }
  }
